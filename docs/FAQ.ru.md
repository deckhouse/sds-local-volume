...
## Когда следует использовать RawFile?

Используйте RawFile когда:

- **Нет выделенных блочных устройств** — вы хотите использовать доступное место на файловой системе вместо настройки LVM
- **Быстрая настройка** — нужно хранилище без конфигурирования ресурсов LVMVolumeGroup
- **Разработка/тестирование** — для непроизводственных окружений, где простота важнее производительности
- **LVM недоступен** — когда в системе не настроен или недоступен LVM

**Избегайте RawFile когда:**

- **Требуется максимальная производительность** — overhead loop-устройства снижает производительность по сравнению с LVM или прямым доступом к блочным устройствам
- **Нужны снимки** — RawFile не поддерживает снимки томов
- **Критична безопасность данных** — `volumeCleanup` не поддерживается для RawFile-томов

Подробнее см. [Использование RawFile-томов](./usage.html#использование-rawfile-томов).
