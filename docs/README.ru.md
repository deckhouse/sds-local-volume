...
{{< alert level="warning" >}}
Возможность работы со снимками томов доступна только в коммерческих редакциях Deckhouse Kubernetes Platform. Для работы со снимками томов требуется подключенный модуль [snapshot-controller](/modules/snapshot-controller/).
{{< /alert >}}
...
- требуется использовать дисковое пространство за счёт выделения томов по требованию (режим LVV-thin).
