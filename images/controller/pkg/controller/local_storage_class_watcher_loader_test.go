...
const (
	testLoaderNamespace  = "d8-sds-local-volume"
	testLoaderSecretName = "d8-sds-local-volume-controller-config"
)
...
func mkSecret(data map[string][]byte) *corev1.Secret {
...
func TestGetStorageClassLabelIgnoredPrefixes_NoConfigKey(t *testing.T) {
	secret := mkSecret(map[string][]byte{"unrelated": []byte("x")})
...
func TestGetStorageClassLabelIgnoredPrefixes_EmptyConfigValue(t *testing.T) {
	secret := mkSecret(map[string][]byte{"config": {}})
	cl := newLoaderFakeClient(t, secret)
	got, err := getStorageClassLabelIgnoredPrefixes(context.Background(), cl, testLoaderNamespace, testLoaderSecretName)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestGetStorageClassLabelIgnoredPrefixes_FieldAbsent(t *testing.T) {
	// Valid YAML without storageClassLabelIgnoredPrefixes — should return nil.
	secret := mkSecret(map[string][]byte{
...
storageClassLabelIgnoredPrefixes:
  ...
  - kubernetes.io/
`)
	secret := mkSecret(map[string][]byte{"config": cfg})
	cl := newLoaderFakeClient(t, secret)
...
func TestGetStorageClassLabelIgnoredPrefixes_MalformedYAML(t *testing.T) {
	// Not a YAML mapping — should produce an error so the caller can requeue
	// instead of silently regressing to "no filtering" behaviour.
	secret := mkSecret(map[string][]byte{
...
// Regression guard for the sigs.k8s.io/yaml gotcha — the library converts
// YAML to JSON and uses encoding/json, which honours `json:` tags (not
// `yaml:`). Renaming the struct field to a name that does NOT match the
// YAML key case-insensitively would silently drop the value if tags ever
// regressed back to `yaml:` — this test pins the JSON-tag contract.
