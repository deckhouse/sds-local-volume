...
var SchedulerExtenderCertConfig = tlscertificate.MustNewGenSelfSignedTLSGroupHookConf(
	tlscertificate.GenSelfSignedTLSHookConf{
		...
		TLSSecretName: "sds-local-volume-scheduler-extender-https-certs",
		SANs: chcrt.DefaultSANs([]string{
			"sds-local-volume--scheduler-extender",
			fmt.Sprintf("sds-local-volume-scheduler-extender.%s", ModuleNamespace),
			fmt.Sprintf("sds-local-volume--scheduler-extender.%s.svc", ModuleNamespace),
			fmt.Sprintf("%%CLUSTER_DOMAIN%%://sds-local-volume--scheduler-extender.%s.svc", ModuleNamespace),
		}),
